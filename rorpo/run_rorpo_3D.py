...
result_jonas = pd.read_csv("Ircad_iso_SS_RORPO_Organ_Best_MCC_per_volume_summary.csv")
dilat_size = 0
nb_core = 4
volumes = []
lengths = []
...
for i in range(0, 20):
    volumes.append(result_jonas.loc[i, "SerieName"].split('/')[1])
    lengths.append(int(result_jonas.loc[i, "VolumeName"].split('-')[0]))
    f_scale.append(float(result_jonas.loc[i, "VolumeName"].split('-')[1]))
    n_scale.append(int(result_jonas.loc[i, "VolumeName"].split('-')[2][0]))
